...
export default function DashboardPage() {
  ...
  // Extract data with safe fallbacks (hooks must be called unconditionally)
  const data = improvedData?.data;
  const preview = data?.resume_preview ?? mockResumeData;
  const newPct = Math.round((data?.new_score ?? 0) * 100);
  const originalPct = Math.round((data?.original_score ?? 0) * 100);
    ...
    () => [
      {
        ...
        ok: Boolean(preview.education && preview.education.length > 0),
      },
      {
        label: 'Work experience section',
        ok: Boolean(preview.workExperience && preview.workExperience.length > 0),
      },
      {
        label: 'Personal projects section',
        ok: Boolean(preview.personalProjects && preview.personalProjects.length > 0),
      },
    ],
                  ...
                  {viewMode === 'resume' ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-300">
                              Mention the company name and (if relevant) its website in your summary
                              or cover letter so ATS tools can tailor job-specific guidance.
                            </p>
                          </div>
                        </div>
                      </div>
                            ...
                            <ul className="text-sm text-gray-300 space-y-1 mt-1">
                              {contactChecks.map((item) => (
                                <li key={item.label} className="flex items-center gap-2">
                                  ...
                                  <span>
                                    {item.ok ? `${item.label} found.` : `${item.label} missing.`}
                                  </span>
                                </li>
                              ))}
                            </ul>
                      ...
                      <div className="rounded-md border border-gray-800 bg-gray-900/70 p-3 space-y-2">
                        <div className="flex items-start gap-3">
                          {getStatusIcon(
                            sectionChecks.every((item) => item.ok) ? 'pass' : 'warning'
                          )}
                          <div className="flex-1">
